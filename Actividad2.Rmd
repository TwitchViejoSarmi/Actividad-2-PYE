...
Para el calculo de la probabilidad teórica definimos nuestra variable con la siguiente notación: Y = Numero de componetes que fallan y $P(Y = 3)$ = probabilidad de que un componente falle(Fallar) o no falle(No fallar), luego mostramos todos los posibles resultados para todos los eventos posibles en una tabla representado a Fallar como 1 y No fallar como 0 (Ver Tabla 1)
...
: Tabla 1: Eventos de los componentes

Como se puede apreciar en la tabla 1, el evento de que los 3 componentes fallen al mismo tiempo tiende a que suceda teóricamente 1 de 8 veces, es decir $P(Y = 3)$ = $\frac{1}{8}$. Luego nos informan que la probabilidad de que pasados 5 años, un componente falle es de 0.1, y como nos dicen que el tiempo de vida de cada componente es independiente cada otro, podemos calcular $P(X = 3)$ de la siguiente manera:

$P(X = 3)$ = $P(Y = 3) \times (0.1)$

$P(X = 3) = \frac{1}{8} \times (0.1)$

$P(X = 3) = 0.0125$

En conclusión la probabilidad teórica de que al seleccionar 3 componentes presenten fallas pasados 5 años es de 0.0125.
